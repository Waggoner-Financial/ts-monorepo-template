...
//
// - Worktrees live at ~/worktrees/<repo>/<dir>/ where <repo> is the main
//   clone's directory name and <dir> is the slug with '/' replaced by '-'.
//   Each worktree owns a port offset stored in <worktree>/.env.worktree. Dev
//   tasks resolve ports as `${WORKTREE_PORT_OFFSET:-0} + <default>` so the
//   main clone (no env file) keeps its default ports unchanged.
...
// - Agent-created worktrees (e.g. under .omx/worktrees/ or /private/tmp/)
...
// The worktree home is derived from the main clone's directory name so the
// template works unchanged for any repo name.
function mainCloneBasename(): string {
  ...
  return first ? basename(first.slice('worktree '.length).trim()) : 'repo';
}

const WORKTREES_HOME = join(homedir(), 'worktrees', mainCloneBasename());

// Port bases. The offset is added to these to get a worktree's actual ports.
// When you add a port-bound dev/E2E service, register its base here so
...
Subcommands:
  new <slug> [--branch <name>] [--base <ref>]
      Create a new worktree at ~/worktrees/<repo>/<slug>.
  ...
  setup [<slug>]
      ...
      ~/worktrees/<repo>/<slug>. Without <slug>, targets the current
      linked worktree.
...
export async function setupWorktree(
  ...
  const slug =
    options.slug ?? existingEnv.WORKTREE_SLUG ?? basename(worktreeRoot);
  const existingOffset = parseOffset(existingEnv.WORKTREE_PORT_OFFSET);
  const offset =
    existingOffset ??
    allocateOffset(
      slug,
  ...
  const envText = `WORKTREE_SLUG=${slug}\nWORKTREE_PORT_OFFSET=${offset}\n`;
  ...
  const flush = () => {
    ...
    if (existsSync(envPath)) {
      try {
        ...
        if (parsed.WORKTREE_PORT_OFFSET !== undefined) {
          const n = Number(parsed.WORKTREE_PORT_OFFSET);
          if (Number.isFinite(n)) offset = n;
        }
        if (parsed.WORKTREE_SLUG !== undefined) {
          slug = parsed.WORKTREE_SLUG;
        }
      } catch {
